...
pub async fn regist_ld_arm1_info(
    ...
    sqlx::query(&format!(
        "INSERT INTO chipdata (machine_id, type_name, lot_name, serial, ld_pickup_date, wano, wax, way,ld_arm1_collet,ld_tray_align_x, ld_tray_align_y)
...
pub async fn regist_uld_arm1_info(
    ...
    sqlx::query(&format!(
        ...
        uld_pre_align_x, uld_pre_align_y, uld_pre_align_t, uld_pocket_align_x, uld_pocket_align_y, uld_pf)
    ...
    .bind(ax).bind(ay).bind(at).bind(pax).bind(pay).bind(pf)
    .execute(&mut **tx).await?;
...
pub async fn regist_uld_chip_info(
    ...
    sqlx::query(
        "INSERT INTO chipdata (machine_id, type_name, lot_name, serial, ld_pickup_date,
         uld_put_date, uld_chip_align_x, uld_chip_align_y, uld_pf)
         VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)
         ON CONFLICT(lot_name, serial, ld_pickup_date, machine_id)
         DO UPDATE SET
    ...
    .bind(machine_id).bind(type_name).bind(lot_name).bind(serial).bind(ld_pickup_date)
    .bind(uld_put_date).bind(cax).bind(cay).bind(pf)
    .execute(&mut **tx).await?;
